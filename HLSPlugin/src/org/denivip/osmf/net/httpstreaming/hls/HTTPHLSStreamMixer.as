...
package org.denivip.osmf.net.httpstreaming.hls
{
	 ...
	 */
	public class HTTPHLSStreamMixer extends EventDispatcher implements IHTTPStreamSource
	{
		/**
		 * Default constructor.
		 */
				...
				if (_mediaTagDataLoaded || _alternateTagDataLoaded)
				{
					...
					else 
					{
						if (_mediaTime > -1 || _alternateTime > -1)
						{
							...
							{
								if(_mediaTime > _alternateTime)
									_alternateTag = null;
								else if(_alternateTime > _mediaTime)
									_mediaTag = null;
							}
		 ...
		 */
		private function shouldFilterTag(tag:FLVTag, filterTags:uint, alternateTag:Boolean = false):Boolean
		{
			if (tag == null)
			{
				return true;
			...
			if (alternateTag && tag.timestamp < _currentTime)
			{
				return true;
			}
		...
		CONFIG::LOGGING
		{
			private static const logger:Logger = Log.getLogger("org.osmf.net.httpstreaming.HTTPHLSStreamMixer");
