...
package org.denivip.osmf.net.httpstreaming.hls
{
	 ...
	 */
	public class HLSStreamingSwitchManager extends RuleSwitchManagerBase
	{
		/**
		 * Constructor.
		 * 
		 ...
		 *  @productversion OSMF 2.0
		 */
		public function HLSStreamingSwitchManager
			...
			else
			{
				...
				CONFIG::LOGGING
				{
					...
					/*if (ExternalInterface.available)
						ExternalInterface.call("console.log", "["+new Date().toLocaleString()+"] Ideal bitrate: " + ABRUtils.roundNumber(idealBitrate) + "; Chosen index: " + index +" Current actual bitrate: "+currentActualBitrate);*/
				}
			}
		...
		protected function getMaxIndex(maxBitrate:Number, emergencyFlag:Boolean = false):uint
		{
			...
			CONFIG::LOGGING
			{
				...
				/*if(ExternalInterface.available)
					ExternalInterface.call("console.log","["+new Date().toLocaleString()+"] Determining the index of the highest quality rendition that has a bitrate smaller or equal than: " + maxBitrate + " kbps");*/
				logSwitchHistory();
			...
			CONFIG::LOGGING
			{
				...
				/*if(ExternalInterface.available)
					ExternalInterface.call('console.log', "["+new Date().toLocaleString()+"] The quality level before applying switch constraints: index = " + newQuality.index + "; bitrate = " + newQuality.bitrate + " kbps");*/
			}
