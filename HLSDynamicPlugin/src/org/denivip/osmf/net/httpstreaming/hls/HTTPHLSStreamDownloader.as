...
package org.denivip.osmf.net.httpstreaming.hls
{
	 ...
	 */
	public class HTTPHLSStreamDownloader extends HTTPStreamDownloader
	{
		/**
		 * Default constructor.
		 * 
		 ...
		 * 					 dispatch any event. 
		 **/
		public function HTTPHLSStreamDownloader()
		{
		}
			...
			if (_urlStream != null)
			{
				...
				CONFIG::LOGGING
				{
					...
					/*if(ExternalInterface.available)
						ExternalInterface.call("console.log", "["+new Date().toLocaleString()+"] Loading (timeout=" + _timeoutInterval + ", retry=" + _currentRetry + "):" + _request.url.toString());*/
				}
		...
		override public function appendToSavedBytes(source:IDataInput, count:uint):void
		{
			...
			CONFIG::LOGGING{
				logger.info('appendToSavedBytes called');
				/*if(ExternalInterface.available){
					ExternalInterface.call("console.log", "["+new Date().toLocaleString()+"] appendToSavedBytes called");
				}*/
			}
		...
		override public function saveRemainingBytes():void
		{
			...
			CONFIG::LOGGING{
				logger.info('saveRemainingBytes called');
				/*if(ExternalInterface.available){
					ExternalInterface.call("console.log", "["+new Date().toLocaleString()+"] saveRemainingBytes called");
				}*/
			}
		...
		private function onComplete(event:Event):void
		{
			...
			CONFIG::LOGGING
			{
				...
				/*if(ExternalInterface.available){
					ExternalInterface.call("console.log", "["+new Date().toLocaleString()+"] Loading complete. It took " + _downloadDuration + " sec and " + _currentRetry + " retries to download " + _downloadBytesCount + " bytes. Url: " + _request.url.toString());
				}*/
			}
...
//				}
		...
		private function onError(event:Event):void
		{
			...
			if(!(event is SecurityErrorEvent) && _request && _dispatcher){
				...
				if(OSMFSettings.hdsMaximumRetries > 0 && _currentRetry < OSMFSettings.hdsMaximumRetries){
					CONFIG::LOGGING {
						logger.error("Error while trying to download [{0}]", _request.url);
						logger.error("Retry");
					}
		...
		CONFIG::LOGGING
		{
			private static const logger:org.osmf.logging.Logger = org.osmf.logging.Log.getLogger("org.denivip.osmf.net.httpstreaming.hls.HTTPHLSStreamDownloader");
