...
package org.denivip.osmf.net.httpstreaming.hls
{
	...
	public class HTTPHLSStreamSource implements IHTTPStreamSource, IHTTPStreamHandler
	{
		...
		protected function doSomeProcessingAndGetBytes():ByteArray
		{
			...
			switch(_state)
			{
				...
				case HTTPStreamingState.LOAD:
					...
					switch(_request.kind)
					{
						...
						case HTTPStreamRequestKind.BEST_EFFORT_DOWNLOAD:
							...
							if (_downloader == null)
							{
								_downloader = new HTTPHLSStreamDownloader();
							}
				...
				case HTTPStreamingState.READ:
					...
					{
						CONFIG::LOGGING{
							logger.info("isOpen="+_downloader.isOpen+" isComplete="+ _downloader.isComplete+" hasData="+ _downloader.hasData+
								" downloadBytesCount="+_downloader.downloadBytesCount+" totalAvailableBytes="+ _downloader.totalAvailableBytes+
								" downloadDuration="+_downloader.downloadDuration);
						}
						
						var msg:String = 'OOOPS...';//"Read some loaded bytes...";//
				...
				case HTTPStreamingState.STOP:
					bytes = _fileHandler.processFileSegment(null);
					if(bytes != null){
						trace("["+new Date().toLocaleString()+"] ----> Sent null bytes to fileHandler. Got "+bytes.length+" bytes processed");
		...
		protected function setState(value:String):void
		{
			...
			if (_state != previouslyLoggedState)
			{
				trace("["+new Date().toLocaleString()+"] StreamSource State = " + _state);
				previouslyLoggedState = _state;
			}
		}
		...
		private var _downloader:HTTPHLSStreamDownloader = null;
		...
		private var _indexDownloader:HTTPHLSStreamDownloader = new HTTPHLSStreamDownloader();
